...
/// This centers the camera, so call it at the end of any 'update' methods
pub fn update_free_camera(camera: &mut FreeCamera, state: &ApplicationState) -> Result<()> {
    let delta_time = state.system.delta_time as f32;
    if state.input.is_key_pressed(VirtualKeyCode::W) {
        camera.translate(CameraDirection::Forward, delta_time);
    }

    if state.input.is_key_pressed(VirtualKeyCode::A) {
        camera.translate(CameraDirection::Left, delta_time);
    }

    if state.input.is_key_pressed(VirtualKeyCode::S) {
        camera.translate(CameraDirection::Backward, delta_time);
    }

    if state.input.is_key_pressed(VirtualKeyCode::D) {
        camera.translate(CameraDirection::Right, delta_time);
    }

    if state.input.is_key_pressed(VirtualKeyCode::LShift) {
        camera.translate(CameraDirection::Down, delta_time);
    }

    if state.input.is_key_pressed(VirtualKeyCode::Space) {
        camera.translate(CameraDirection::Up, delta_time);
    }

    let offset = state.input.mouse.offset_from_center;
